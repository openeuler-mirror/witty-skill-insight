...
description: Use when optimizing Agent Skill definitions via static compliance checks, LLM quality evaluations, runtime trace crystallization, or direct user revision requests. Supports static/dynamic/feedback modes with snapshot versioning and interactive review loops.
---
...
本技能用于优化 Agent Skill 定义（`SKILL.md` 及其辅助文件）。支持三种原子优化模式，可单独执行或由 Agent 编排为顺序流程。
