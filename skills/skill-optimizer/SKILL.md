...
description: Use when optimizing Agent Skill definitions via static compliance checks, LLM quality evaluations, runtime trace crystallization, Trace2Skill trace analysis, or direct user revision requests. Supports static/dynamic/hybrid/feedback/trace modes with snapshot versioning and interactive review loops.
---
...
本技能用于优化 Agent Skill 定义（`SKILL.md` 及其辅助文件）。支持五种原子优化模式，可单独执行或由 Agent 编排为顺序流程。
...
| `trace`    | Trace2Skill 深度优化（arXiv:2603.25158） | 有轨迹池数据，想通过层次化分析改进 |
...
./scripts/opt.sh --action optimize --mode trace    --input /path/to/skill_dir --trajectories /path/to/trajectories_dir
```

> **Trace2Skill 模式说明**：`trace` 模式基于 arXiv:2603.25158 论文实现，需要提供轨迹数据目录。每个轨迹文件应为 JSON 格式，包含 task_id、query、steps、final_output、status（success/failure）等字段。
